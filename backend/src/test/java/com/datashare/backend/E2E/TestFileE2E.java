...
public class TestFileE2E {
	...
	void fullDownloadClien() throws Exception {
		...
		String LoginResponse = mockMvc.perform(post("/api/login").contentType(MediaType.APPLICATION_JSON).content("""
				{
				"login":"mario",
				"password":"pass123"
				}
		...
		String donloadToken= mapper
				.readTree(UploadResponse)
				.get("downloadToken")
				.asText();
		
		 mockMvc.perform(get("/api/files/download/" + donloadToken))
         .andExpect(status().isOk())
